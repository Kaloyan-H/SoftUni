...
namespace Stealer
{
    public class Spy
    {
        ...
        private MethodInfo[] GetMethodInfos(Type classType)
        {
        ...
        public string CollectGettersAndSetters(string className)
        {
            ...
            MethodInfo[] methodInfos = GetMethodInfos(classType);

            StringBuilder sb = new StringBuilder();

            foreach (var methodInfo in methodInfos.Where(m => m.Name.StartsWith("get_")))
            {
                sb.AppendLine($"{methodInfo.Name} will return {methodInfo.ReturnType.FullName}");
            }
            foreach (var methodInfo in methodInfos.Where(m => m.Name.StartsWith("set_")))
            {
                sb.AppendLine($"{methodInfo.Name} will set field of " +
                    $"{methodInfo.GetParameters().First().ParameterType.FullName}");
            }
